...
class TempSequenceDataset(Dataset):
    def __init__(self, data_dir, sequence_length=10, scaler_path="models/scaler.save"):
        ...
        for file_path in train_files:
            ...
            if "Time" in df.columns:
                df["Time"] = df["Time"] - df["Time"].min()  # Shift time to start from 0

            thermal_cols = [col for col in df.columns if col.startswith("TC")]
            thermal_data_all.append(df[thermal_cols])
            param_data_all.append(df[["h", "flux", "abs", "surf", "Time"]].iloc[0])
        ...
        for thermal_df, param_row in zip(thermal_data_all, param_data_all):
            ...
            scaled_param = self.param_scaler.transform([param_row])[0]
            time_scaled = scaled_param[-1]
            other_params = scaled_param[:-1]
            ...
            for i in range(len(scaled_temp) - sequence_length):
                ...
                X_seq_with_params = [list(row) + other_params.tolist() + [time_scaled] for row in X_seq]
    ...
    def get_test_dataset(self):
        ...
        for file_path in self.test_files:
            ...
            scaled_param = self.param_scaler.transform(df[["h", "flux", "abs", "surf", "Time"]].iloc[0].values.reshape(1, -1))[0]
            time_scaled = scaled_param[-1]
            other_params = scaled_param[:-1]
