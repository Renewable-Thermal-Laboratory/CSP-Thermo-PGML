...
class TempSequenceDataset(Dataset):
    def __init__(self, data_dir, sequence_length=10, scaler_dir="models"):
        ...
        self.scaler_dir = scaler_dir  # Define scaler_dir as instance variable
        os.makedirs(self.scaler_dir, exist_ok=True)  # Use self.scaler_dir
    ...
    def _create_sequences(self, df, sample_list):
        ...
        for i in range(len(thermal_data) - self.sequence_length):
            ...
            # Add parameters to each timestep
            seq_with_params = np.hstack([
                sequence,
                np.tile(params[:-1], (sequence.shape[0], 1)),  # Static params
                np.linspace(0, params[-1], sequence.shape[0])[:, None]  # Time progression
            ])
